...
public class MobPullManager(JobViewWindow qtInstance, string holdQT = "") {
  ...
  public void 拉怪中留爆发(int 当前时间, float 集中度) {
    HoldBurstIfPulling(当前时间, 集中度);
  }
  ...
  private void SetAllQTs(bool val) {
    if (HoldQtName.IsNullOrEmpty()) {
      foreach (string item in BurstQTs) {
        qtInstance.SetQt(item, val);
      }
    }
  ...
  private float CheckEnemiesAroundTank() {
    ...
    if (_currTank == null) return 0f;

    var enemies = TargetMgr.Instance.Enemys;

    foreach (var item in enemies) {
      if (_currTank.Distance(item.Value) <= 25f) visibleEnemiesIn25++;

      if (_currTank.Distance(item.Value) <= 5f) visibleEnemiesIn5++;
    }

    return visibleEnemiesIn25 > 0 ? visibleEnemiesIn5 / (float)visibleEnemiesIn25 : 0f;
  }
  ...
  public static float 附近敌人总血量比例() {
    return GetTotalHealthPercentageOfNearbyEnemies();
  }
  ...
  public static float 附近敌人平均死亡时间() {
    return GetAverageTTKOfNearbyEnemies();
  }
