...
public class QtStyle {
  ...
  public bool CurrentThemeChanged {
    get {
      bool changed = _lastTheme != Save.CurrentTheme;
      return changed;
    }
  }
  ...
  public QtStyle(JobViewSave save) {
    ...
    ModernTheme.ThemePreset savedTheme = save.CurrentTheme;
    ModernTheme = new ModernTheme(savedTheme);
    _lastTheme = savedTheme; // 初始化lastTheme
  }

  public static float OverlayScale => 1f.Scale();
  ...
  public const ImGuiWindowFlags QtWindowFlag = ImGuiWindowFlags.NoScrollbar
  ...
  public void Reset() {
    Save.CurrentTheme = ModernTheme.ThemePreset.BLM;
    _lastTheme = ModernTheme.ThemePreset.BLM;
    ModernTheme.ApplyPreset(ModernTheme.ThemePreset.BLM);
