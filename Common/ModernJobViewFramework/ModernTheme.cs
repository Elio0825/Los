...
public class ModernTheme {
  ...
  // 圆角半径
  private float _borderRadius { get; set; } = 8f;

  // 预设主题
  private static readonly Dictionary<ThemePreset, ColorScheme> _presets = new() {
      ...
      [ThemePreset.BLM] = new ColorScheme {
          ...
          TextSecondary = new Vector4(0.85f, 0.85f, 0.85f, 1f),
          Success = new Vector4(0.4f, 0.9f, 0.6f, 1f),
          Warning = new Vector4(1f, 0.7f, 0.5f, 1f),
          Error = new Vector4(1f, 0.5f, 0.5f, 1f),
          Border = new Vector4(0.3f, 0.6f, 0.6f, 0.3f),
          Shadow = new Vector4(0.1f, 0f, 0.05f, 0.6f),
      },
  };
  ...
  public void Apply() {
    ...
    // 窗口圆角
    style.WindowRounding = _borderRadius;
    style.ChildRounding = _borderRadius;
    style.FrameRounding = _borderRadius * 0.5f;
    style.PopupRounding = _borderRadius;
    style.ScrollbarRounding = _borderRadius * 0.5f;
    style.GrabRounding = _borderRadius * 0.5f;
    style.TabRounding = _borderRadius * 0.5f;
  ...
  public void Restore() {
    ...
    // 恢复颜色
    foreach (var kvp in _savedColors) style.Colors[(int)kvp.Key] = kvp.Value;
  ...
  /// 绘制渐变背景
  /// </summary>
  public static void DrawGradient(Vector2 pos,
                                  Vector2 size,
                                  Vector4 colorTop,
                                  Vector4 colorBottom) {
  ...
  public static Vector4 BlendColor(Vector4 color1, Vector4 color2, float ratio) {
    return new Vector4(color1.X * (1f - ratio) + color2.X * ratio,
