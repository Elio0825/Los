...
public static class BLMHelper
{
    ...
    {
        if (Core.Me.Level >= 92)
            return Helper.目标Buff时间小于(Buffs.雷一dot, 3500, false) && Helper.目标Buff时间小于(Buffs.雷二dot, 3500, false);
        if (Core.Me.Level >= 45)
            return Helper.目标Buff时间小于(Buffs.暴雷, 3500, false) && Helper.目标Buff时间小于(Buffs.霹雳, 3500, false);
        return false;
    }
    ...
    {
        if (Core.Me.Level >= 92)
            return Helper.目标Buff时间小于(Buffs.雷一dot, 6000, false) && Helper.目标Buff时间小于(Buffs.雷二dot, 6000, false);
        if (Core.Me.Level >= 45)
            return Helper.目标Buff时间小于(Buffs.暴雷, 6000, false) && Helper.目标Buff时间小于(Buffs.霹雳, 6000, false);
        return false;
    }
    ...
    public static uint 可用瞬发()
    {
        ...
        if (补dot() && Helper.有buff(Buffs.雷云buff)) return aoe ? Skill.雷二.GetActionChange() : Skill.雷一.GetActionChange();
        if (火状态 && 火层数 < 3 && Helper.有buff(Buffs.火苗buff) && !三目标aoe()) return Skill.火三;
        if (悖论指示)
        {
        ...
        if (提前补dot() && Helper.有buff(Buffs.雷云buff))
            return aoe ? Skill.雷二.GetActionChange() : Skill.雷一.GetActionChange();
        if (Skill.即刻.GetSpell().Cooldown.TotalMilliseconds > 0 && Skill.三连.GetSpell().Charges < 1)
        {
            if (Helper.有buff(Buffs.火苗buff) && BLMHelper.火状态) return Skill.火三;
            if (Helper.有buff(Buffs.雷云buff)) return aoe ? Skill.雷二 : Skill.雷一;
        }
